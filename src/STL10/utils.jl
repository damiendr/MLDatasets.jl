...
Convert the given STL-10 tensor to a feature matrix (or feature
...
```julia
julia> STL10.convert2features(STL10.traintensor(Float32)) # full training data
3072×50000 Array{Float32,2}:
[...]

julia> STL10.convert2features(STL10.traintensor(Float32,1)) # first observation
...
Convert the given STL-10 horizontal-major tensor (or feature
vector/matrix) to a vertical-major `RGB` array.

```julia
julia> STL10.convert2image(STL10.traintensor()) # full training dataset
32×32×50000 Array{RGB{N0f8},3}:
[...]

julia> STL10.convert2image(STL10.traintensor(1)) # first training image
32×32 Array{RGB{N0f8},2}:
[...]
```
"""
function convert2image(array::AbstractVector{<:Number})
    @assert length(array) % NUM_FEATURES == 0
    if length(array) == NUM_FEATURES
        convert2image(reshape(array, IMAGE_SIZE..., 3))
    else
        n = Int(length(array) / (NUM_FEATURES))
        convert2image(reshape(array, IMAGE_SIZE..., 3, n))
    end
end

function convert2image(array::AbstractMatrix{<:Number})
    @assert size(array, 1) == NUM_FEATURES
    convert2image(reshape(array, IMAGE_SIZE..., 3, size(array, 2)))
end
